...
class Heap(BinaryTree):
    ...
    def is_heap_satisfied(self):
        ...
        This makes it possible to automatically test if insert/delete functions
        are really working.
        '''
        if self.root:
    ...
    def insert(self, value):
        ...
        in a variable (rather than compute it) to maintain the O(log n) runtime
        ...
        followin the same pattern used in the BST and AVLTree insert functions.
        '''
        self.num_nodes += 1
    ...
    def remove_min(self):
        ...
        HINT:
        I created 2 @staticmethod functions: _remove_bottom_right and _trickle.
        It's possible to do it with only a single helper (or no helper at all),
        but I personally found div up the code into two made the most sense.
        '''
        if not self.root:
            return
        else:
            binary_str = bin(self.num_nodes)[3:]
            self._remove_bottom_right(self, self.root, binary_str)
            self.num_nodes -= 1

    @staticmethod
    def _remove_bottom_right(heap, node, binary_str):
        if not binary_str:
            return node
        else:
            if binary_str[0] == '0':
            ...
            if binary_str[0] == '0' and len(binary_str) == 1:
                if node.right:
                    node.value = heap.find_smallest()
                    node.value, node.right.value = node.right.value, node.value
                    node.right = None
                else:
                    node.value = heap.find_smallest()
                    node.value, node.left.value = node.left.value, node.value
                    node.left = None
            elif binary_str[0] == '1' and len(binary_str) == 1:
                if node.right:
                    node.value = heap.find_smallest()
                    node.value, node.right.value = node.right.value, node.value
                    node.right = None
                ...
                Heap._remove_bottom_right(heap, next_node, binary_str[1:])
        Heap._trickle(node)
